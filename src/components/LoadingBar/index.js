...
import { DEFAULT_SCOPE } from "../../reducers/loadingBar";
...
class LoadingBar extends Component {
  ...
  stop() {
    ...
    const terminatingAnimationDuration =
      this.isShown() || showFastActions ? TERMINATING_ANIMATION_DURATION : 0;
  ...
  buildStyle() {
    ...
    if (!className) {
      style.height = "2px";
      style.background =
        "linear-gradient(190deg, rgb(255, 0, 0), rgb(255, 110, 0))";
...
const mapStateToProps = (state, ownProps) => ({
  loading: state.loadingBar[DEFAULT_SCOPE],
});
